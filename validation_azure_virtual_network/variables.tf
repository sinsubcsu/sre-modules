...
variable "vnet_name" {
  ...
  validation {
    # Set the correct regex for the naming convention
    condition     = length(regexall("-", var.vnet_name)) > 0 ? false : true
    error_message = "vnet name cannot contain the following characters '-'"
  }
}
output "vnet_name" {
  value = var.vnet_name
